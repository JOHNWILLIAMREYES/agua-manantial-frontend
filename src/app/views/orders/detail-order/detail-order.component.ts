...
export class DetailOrderComponent implements OnInit {
  ...
  getActiveTicketHolder() {
    this.ticketService
      .getActiveTicketHolder('active-ticket-holder')
      .subscribe((res) => {
  ...
  createTicket() {
    this.ticketService
      ...
      .subscribe((response) => {
        if (
          this.ticket_holder.max_number -
            (Number(response.secuence.split('-')[1]) + 1) <=
          10
        ) {
          Swal.fire({
            title:
              'Estás a 10 facturas o menos de alcanzar el rango máximo del facturero aprobado por la DIAN',
            icon: 'info',
          });
        }
        const data = {
          order: this.order,
          ticketNumber: `${this.ticket_holder.prefix}-${
            Number(response.secuence.split('-')[1]) + 1
          }`,
        };
